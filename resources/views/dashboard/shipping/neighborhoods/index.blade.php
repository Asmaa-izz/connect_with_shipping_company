...
@section('content')
            ...
            <div class="card">
                <div class="card-body">
                    <div class="card-title d-flex justify-content-between align-items-center my-3">
                        <h4>جميع الأحياء</h4>
                        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#creat-neighborhood" id="create">
                            إضافة حي جديد
                        </button>
                    </div>
    ...
    <div class="modal fade" id="creat-neighborhood" tabindex="-1" role="dialog" aria-labelledby="creatRoleLabel"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="creatRoleLabel">اضافة حي جديدة</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="form-create" action="{{ route('neighborhoods.store') }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label for="name"  class="required form-label">اسم الحي: </label>
    ...
    <div class="modal fade" id="edit-neighborhood" tabindex="-1" role="dialog" aria-labelledby="creatRoleLabel"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="creatRoleLabel">تعديل حي</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form id="form-edit" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="name_edit"  class="required form-label">اسم الحي </label>
                            <input type="text" class="form-control" name="name" id="name_edit">
                        </div>

                        <div class="mb-3">
                            <label for="city_edit" class="required">المحافظة :</label>
                            <select class="select2 form-control" required
                                    data-placeholder="اختر " name="city" id="city_edit">
                                @foreach($cities as $city)
                                    <option value="{{$city->id}}">{{$city->name}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            <label for="area_edit" class="required">المنطقة :</label>
                            <select class="select2 form-control" required
                                    data-placeholder="اختر المرشد" name="area" id="area_edit">
                            </select>
                        </div>
                    </div>
                    <div class="modal-footer">
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function deleteItem(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch(`/dashboard/neighborhoods/${id}`, {
                        headers: {
        ...
        $(document).on("click", ".edit-item", function () {
            ...
            $("#name_edit").val(neighborhoodName);
            $('#form-edit').attr('action', '/dashboard/neighborhoods/' + neighborhoodId);

            $('#city_edit').val(cityId).trigger('change');
            $('#area_edit').val(areaId).trigger('change');
        });
